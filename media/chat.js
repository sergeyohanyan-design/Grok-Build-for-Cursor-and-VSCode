(function () {
  ...
  const state = {
    ...
    busyLocked: false,
    // Session-start window: welcome shows "Starting" until setBusy:false clears it.
    startingPhase: false,
    cliVersion: "",
  ...
  const PRIMER_PATTERN = /^\s*\[grok-build-(?:vscode|gui) primer v\d+\]/;
  ...
  const ICON = {
    ...
    selection: `<svg xmlns="http://www.w3.org/2000/svg" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M4 7h16"/><path d="M4 12h10"/><path d="M4 17h14"/></svg>`,
    ...
    image: `<svg xmlns="http://www.w3.org/2000/svg" width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect width="18" height="18" x="3" y="3" rx="2"/><circle cx="9" cy="9" r="2"/><path d="m21 15-3.086-3.086a2 2 0 0 0-2.828 0L6 21"/></svg>`,
  ...
  function updateModeBtn(modeId) {
    ...
    if (modeBtn) {
      const pillLabel = meta.shortLabel || meta.label;
      modeBtn.innerHTML =
        `${meta.icon}<span class="btn-label">${escapeHtml(pillLabel)}</span><span class="pill-chevron">▾</span>`;
  ...
  function updateModelPill() {
    const name = modelDisplayName(state.currentModelId, state.availableModels) || "Grok Build";
    if (modelLabel) modelLabel.textContent = truncate(name, 16);
    if (modelBtn) {
      modelBtn.title = state.busy
        ? `${name} — available once ready`
        : `Model: ${name} — click to change`;
      modelBtn.classList.toggle("disabled", state.busy);
    }
  }
  ...
  if (actionBtn) actionBtn.innerHTML = ICON.mic;
  gearBtn.innerHTML = ICON.gear;
  if (addBtn) {
    addBtn.innerHTML = `${ICON.plus}<span class="btn-label">Context</span>`;
    addBtn.classList.add("footer-pill", "context-toolbar-btn");
  }
  if (uploadBtn) uploadBtn.innerHTML = ICON.upload;
  if (dropOverlay) {
    const iconEl = dropOverlay.querySelector(".drop-overlay-icon");
    if (iconEl) iconEl.innerHTML = ICON.upload;
  }
  updateModeBtn("agent");
  updateModelPill();
  ...
  function closePopovers() {
    ...
    if (modelBtn) modelBtn.classList.remove("popover-open");
    if (addBtn) addBtn.classList.remove("popover-open");
  }
  ...
  function positionPopover(popover, btn) {
    ...
    popover.style.bottom = (parentRect.bottom - btnRect.top + 6) + "px";
    popover.style.left = (btnRect.left - parentRect.left) + "px";
    popover.style.right = "auto";
    popover.style.maxHeight = Math.min(480, Math.max(220, spaceAbove)) + "px";
    if (popover.classList.contains("gear-popover")) {
      popover.style.minWidth = "268px";
    } else if (popover.classList.contains("model-popover")) {
      popover.style.minWidth = "280px";
    } else if (popover.classList.contains("history-popover")) {
    ...
    requestAnimationFrame(() => {
      ...
      if (leftOffset + pw > parentRect.width) {
        popover.style.left = Math.max(4, parentRect.width - pw - 4) + "px";
      }
    });
  }
  ...
  function renderModelPicker() {
    ...
    appendModelList(gearPopover, "");
  }

  function filterModels(query) {
    const all = state.availableModels.length
  ...
  function appendModelList(container, query) {
    const models = filterModels(query);
    const settingsLocked = state.busy;
    if (!models.length) {
    ...
    for (const m of models) {
      ...
      const el = document.createElement("div");
      el.className = "model-card" + (active ? " active" : "") + (settingsLocked ? " disabled" : "");
      const ctx = formatContextTokens(m.totalContextTokens);
      el.innerHTML =
        `<span class="model-card-radio" aria-hidden="true"></span>` +
        `<div class="model-card-content">` +
          `<span class="model-card-name">${escapeHtml(m.name || m.modelId)}</span>` +
          `<span class="model-card-meta">` +
            (ctx ? `<span class="model-card-ctx">${escapeHtml(ctx)}</span>` : "") +
            `<span class="model-card-id muted">${escapeHtml(m.modelId)}</span>` +
          `</span>` +
        `</div>` +
        (active ? '<span class="model-card-badge">Active</span>' : "");
      if (!settingsLocked) {
        el.onclick = (e) => {
          e.stopPropagation();
  ...
  function renderModelListBody() {
    ...
    if (!list) return;
    list.innerHTML = "";
    appendModelList(list, state.modelSearch);
  }

  function renderEffortRow(container) {
    const section = document.createElement("div");
    section.className = "popover-section";
    section.textContent = "Reasoning effort";
    container.appendChild(section);
    const row = document.createElement("div");
    row.className = "effort-row-expanded";
    const settingsLocked = state.busy;
    const currentIdx = EFFORT_LEVELS.indexOf(state.effort);
    EFFORT_LEVELS.forEach((id, i) => {
      const btn = document.createElement("button");
      btn.type = "button";
      btn.className = "effort-pill" + (i <= currentIdx ? " active" : "") + (settingsLocked ? " disabled" : "");
      btn.textContent = capitalize(id);
      btn.title = EFFORT_TOOLTIPS[id] || capitalize(id);
      if (!settingsLocked) {
        btn.onclick = (e) => {
          e.stopPropagation();
          state.effort = state.effort === id ? "" : id;
  ...
  function openModelPopover() {
    ...
    if (!modelPopover.hidden) {
      closePopovers();
      modelBtn.classList.remove("popover-open");
      return;
    }
    closePopovers();
    state.modelSearch = "";
    modelPopover.innerHTML = "";
    const currentName = modelDisplayName(state.currentModelId, state.availableModels) || "Grok Build";

    const header = document.createElement("div");
    header.className = "popover-header";
    header.innerHTML =
      `<div class="popover-header-title">Select model</div>` +
      `<div class="popover-header-sub muted">Current · ${escapeHtml(currentName)}</div>`;
    ...
    search.value = "";
    search.oninput = () => {
      state.modelSearch = search.value;
      renderModelListBody();
    };
    ...
    modelPopover.appendChild(searchWrap);
    modelPopover.appendChild(list);
    renderModelListBody();
    renderEffortRow(modelPopover);
    positionDropdownPopover(modelPopover, modelBtn);
    modelPopover.hidden = false;
    modelBtn.classList.add("popover-open");
  ...
  function openModePopover() {
    ...
    const modeAnchor = modeBtn;
    positionPopover(modePopover, modeAnchor);
    modePopover.hidden = false;
  }

  function addContextItem(icon, label, desc, onClick, opts) {
    opts = opts || {};
    const item = document.createElement("div");
    item.className = "context-menu-item";
    item.innerHTML =
      `<span class="context-menu-icon">${icon}</span>` +
      `<span class="context-menu-body">` +
        `<span class="context-menu-label">${escapeHtml(label)}</span>` +
        (desc ? `<span class="context-menu-desc muted">${escapeHtml(desc)}</span>` : "") +
      `</span>` +
      (opts.shortcut ? `<kbd class="context-menu-kbd">${escapeHtml(opts.shortcut)}</kbd>` : "") +
      `<span class="context-menu-chevron" aria-hidden="true">›</span>`;
    item.onclick = (e) => { e.stopPropagation(); onClick(); closePopovers(); };
    addPopover.appendChild(item);
  }

  function openAddPopover() {
    if (!addPopover.hidden) {
      closePopovers();
      if (addBtn) addBtn.classList.remove("popover-open");
      return;
    }
    ...
    header.className = "popover-header";
    header.innerHTML =
      `<div class="popover-header-title">Add context</div>` +
      `<div class="popover-header-sub muted">Files, images, and code for Grok Build</div>`;
    addPopover.appendChild(header);

    const section = document.createElement("div");
    section.className = "popover-section";
    section.textContent = "Attach";
    addPopover.appendChild(section);

    addContextItem(ICON.upload, "Upload files", "Images, PDFs, code, folders from disk", () => {
      vscode.postMessage({ type: "pickFile" });
    });
    addContextItem(ICON.file, "Active editor file", "Include the file open in your editor", () => {
      vscode.postMessage({ type: "attachActiveFile" });
    });
    addContextItem(ICON.selection, "Selected lines", "Attach only the highlighted selection", () => {
      vscode.postMessage({ type: "attachActiveSelection" });
    });
    addContextItem(ICON.folder, "Browse workspace", "Pick files from this project", () => {
      vscode.postMessage({ type: "pickFile" });
    });

    const hint = document.createElement("div");
    hint.className = "context-drop-hint muted";
    hint.innerHTML = `<span class="context-drop-icon">${ICON.upload}</span> Drop files onto the chat to attach instantly`;
    addPopover.appendChild(hint);

    const anchor = addBtn || uploadBtn;
    positionPopover(addPopover, anchor);
    addPopover.hidden = false;
    if (addBtn) addBtn.classList.add("popover-open");
  }
  ...
  function clearWelcome() {
    ...
    if (welcome) {
      ...
      welcome.classList.add("welcome-dismissed");
      welcome.hidden = true;
    }
    state.welcomeVisible = false;
  }

  function fadeWelcomeOnFirstMessage() {
    if (!state.welcomeVisible || state.hasUserSent) return;
    state.hasUserSent = true;
    const welcome = $("welcome");
    if (!welcome) return;
    welcome.classList.add("welcome-fade-out");
    setTimeout(() => clearWelcome(), 400);
  }

  function restoreWelcomeScreen() {
    const welcome = $("welcome");
    if (!welcome) return;
    welcome.classList.remove("welcome-dismissed", "welcome-fade-out");
    welcome.hidden = false;
    const loader = $("welcome-loader");
    if (loader) loader.classList.remove("done");
    const ver = $("welcome-version");
    if (ver) {
      ver.hidden = false;
      ver.classList.add("loading-dots");
      ver.classList.remove("connected", "error");
      ver.textContent = "Starting";
    }
    ...
    state.hasUserSent = false;
    state.optimisticUserText = null;
  }

  function showAgentPending() {
    if (state.agentPendingEl || state.suppressReplayTurn) return;
    const el = document.createElement("div");
    el.className = "agent-pending";
    el.innerHTML =
      `<div class="agent-pending-inner">` +
        `<div class="agent-pending-avatar">` +
          `<span class="agent-pending-mark">${GROK_MARK}</span>` +
          `<span class="agent-pending-ring" aria-hidden="true"></span>` +
        `</div>` +
        `<div class="agent-pending-body">` +
          `<span class="agent-pending-label loading-dots">Thinking</span>` +
          `<div class="thinking-wave" aria-hidden="true">` +
            `<span></span><span></span><span></span><span></span><span></span>` +
          `</div>` +
        `</div>` +
        `<div class="agent-pending-shimmer"></div>` +
      `</div>`;
    messagesEl.appendChild(el);
    state.agentPendingEl = el;
    scrollToBottom();
  }

  function hideAgentPending() {
    if (!state.agentPendingEl) return;
    state.agentPendingEl.remove();
    state.agentPendingEl = null;
  }

  function setConnectionStatus(status, info) {
    info = info || {};
    ...
    if (status === "connecting") {
      if (verEl) {
        ...
        verEl.classList.remove("connected", "error");
        verEl.textContent = info.updating ? "Updating Grok Build CLI" : "Starting";
      }
      ...
      if (info.version) state.cliVersion = info.version;
      state.startingPhase = false;
      ...
      if (state.connectTimer) { clearTimeout(state.connectTimer); state.connectTimer = null; }
      if (verEl) {
        verEl.classList.remove("loading-dots", "connected");
        verEl.classList.add("error");
        verEl.textContent = info.message || "Connection failed";
      }
      if (loader) loader.classList.remove("active");
      const onb = $("welcome-onboarding");
      if (onb && !onb.innerHTML.trim()) {
        onb.innerHTML =
          `<div class="onb">` +
            `<p class="onb-desc">${escapeHtml(info.message || "Could not connect to Grok CLI")}</p>` +
            `<button class="onb-action" type="button" data-act="recheck">Retry connection</button>` +
          `</div>`;
      }
    }
  }
  ...
  function addMessage(role, text, chips) {
    if (role === "user") fadeWelcomeOnFirstMessage();
    const el = document.createElement("div");
    ...
    if (role === "user" && chips && chips.length > 0) {
      ...
      for (const chip of chips) {
        ...
        tag.innerHTML = `<span class="msg-chip-icon">${ICON.file}</span><span>${escapeHtml(truncate(fileName, 22))}</span>`;
  ...
  function closeToolGroup() {
    ...
    if (calls.length === 1) {
      ...
      const chev = hdr.querySelector(".tool-chevron");
  ...
  function addPermissionCard(req) {
    clearWelcome();
    const el = document.createElement("div");
    el.className = "card permission perm-card";

    const panel = document.createElement("div");
    panel.className = "perm-card-panel";

    const title = document.createElement("div");
    title.className = "perm-card-title";
    const rawTitle = req.toolCall?.title || `permission: ${req.toolCall?.kind || "tool"}`;
    const codeMatch = rawTitle.match(/`([^`]+)`/);
    if (codeMatch) {
      const before = rawTitle.slice(0, codeMatch.index);
      const after = rawTitle.slice((codeMatch.index || 0) + codeMatch[0].length);
      title.innerHTML =
        `${escapeHtml(before)}<code class="perm-path">${escapeHtml(codeMatch[1])}</code>${escapeHtml(after)}`;
    } else {
    ...
    if (diff) {
      ...
      subtitle.className = "perm-card-subtitle";
      const oldLines = (diff.oldText || "").split("\n").length;
      const newLines = (diff.newText || "").split("\n").length;
      ...
      const preview = document.createElement("button");
      preview.type = "button";
      preview.className = "preview-link perm-preview-link";
      preview.textContent = "open diff preview →";
      preview.onclick = () =>
        vscode.postMessage({
          type: "openDiff",
    ...
    const actions = document.createElement("div");
    actions.className = "card-actions perm-card-actions";
    for (const opt of req.options || []) {
      const btn = document.createElement("button");
      btn.type = "button";
      btn.textContent = opt.name;
      if (opt.kind === "allow_always") btn.classList.add("perm-session");
      if (opt.kind === "allow_once") btn.classList.add("perm-yes");
      if (opt.kind === "reject_once") btn.classList.add("perm-no");
      btn.onclick = () => {
        vscode.postMessage({
  ...
  function renderChips() {
    chipsEl.innerHTML = "";
    const visible = state.chips.filter((c) => !c.hidden);
    for (const chip of state.chips) {
      const el = document.createElement("div");
      const img = isImagePath(chip.path);
      el.className = "chip" + (chip.hidden ? " chip-hidden" : "") + (img ? " chip-image" : "");
      el.title = chip.path;
      const fileName = (chip.relPath.split("/").pop() || chip.relPath);
      el.innerHTML = (chip.hidden ? ICON.eyeOff : (img ? ICON.image : ICON.file)) +
        `<span>${truncate(fileName, 18)}</span>`;
      el.onclick = () => vscode.postMessage({ type: "toggleChip", id: chip.id });
      chipsEl.appendChild(el);
    }
    if (chipsRow) chipsRow.hidden = state.chips.length === 0;
    if (addBtn) {
      const n = visible.length;
      addBtn.dataset.count = n > 0 ? String(n) : "";
      addBtn.title = n > 0 ? `${n} file${n === 1 ? "" : "s"} attached` : "Add context";
    }
    updateActionButton();
  }
  ...
  function updateDonut(used) {
    const max = Math.max(1, Number(state.contextWindow) || 200000);
  ...
  function hasComposerContent() {
    return !!input.value.trim() || state.chips.some((c) => !c.hidden);
  }

  function updateActionButton() {
    if (!actionBtn) return;
    actionBtn.classList.remove(
      "stop", "initializing", "mic-mode", "send-mode",
      "listening", "transcribing", "connecting", "needs-setup",
    );
    if (state.busy) {
      if (state.busyLocked) {
        actionBtn.innerHTML = ICON.spinner;
        actionBtn.title = "Initializing…";
        actionBtn.classList.add("initializing");
        actionBtn.disabled = true;
        return;
      }
      actionBtn.innerHTML = ICON.square;
    ...
    if (state.mic === "listening") {
      actionBtn.innerHTML = ICON.micWaves;
      actionBtn.title = "Listening — click to stop";
      actionBtn.classList.add("listening");
      actionBtn.disabled = false;
      return;
    }
    if (state.mic === "connecting") {
      actionBtn.innerHTML = ICON.spinner;
      actionBtn.title = "Starting mic…";
      actionBtn.classList.add("connecting");
      actionBtn.disabled = false;
      return;
    }
    if (state.mic === "transcribing") {
      actionBtn.innerHTML = ICON.spinner;
      actionBtn.title = "Transcribing…";
      actionBtn.classList.add("transcribing");
      actionBtn.disabled = true;
      return;
    }
    if (hasComposerContent()) {
      actionBtn.innerHTML = ICON.arrowUp;
  ...
  function onActionClick() {
    ...
    if (state.mic === "listening" || state.mic === "connecting") {
      toggleMic();
      return;
    }
    if (hasComposerContent()) {
  ...
  function sendOrStop() {
    ...
    fadeWelcomeOnFirstMessage();
    state.optimisticUserText = text;
    addMessage("user", text, visibleChips);
    state.busy = true;
    updateActionButton();
    state.activeAgentEl = null;
    state.activeAgentRaw = "";
  ...
  function submitVoiceMessage(text) {
    ...
    if (!t) return;
    fadeWelcomeOnFirstMessage();
    state.optimisticUserText = t;
    addMessage("user", t, state.chips.filter((c) => !c.hidden));
    state.busy = true;
    updateActionButton();
    showAgentPending();
    state.activeAgentEl = null;
  ...
  window.addEventListener("message", (e) => {
    ...
    switch (msg.type) {
      ...
      case "cliUpdating": {
        setConnectionStatus("connecting", { updating: true });
        break;
      }
      case "connectionState":
        if (msg.state === "connecting") setConnectionStatus("connecting", { updating: msg.updating });
        else if (msg.state === "connected") setConnectionStatus("connected", { version: msg.version });
        else if (msg.state === "error") setConnectionStatus("error", { message: msg.message });
        break;
      case "session": {
        ...
        updateModelPill();
        if (state.connectionState !== "connected") {
          setConnectionStatus("connected", { version: state.cliVersion });
        }
        break;
      }
      case "modelChanged":
      ...
      case "userMessage":
        ...
        drainPlanHistory(state.userMsgCount);
        state.userMsgCount += 1;
        if (state.optimisticUserText !== null && msg.text === state.optimisticUserText) {
          state.optimisticUserText = null;
        } else {
          addMessage("user", msg.text, msg.chips || []);
        }
        if (!state.agentPendingEl && state.busy) showAgentPending();
      ...
      case "setBusy":
        ...
        if (modelPopover && !modelPopover.hidden) openModelPopover();
        break;
      case "summarizing": {
        clearWelcome();
  ...
  if (actionBtn) {
    actionBtn.onclick = (e) => { e.stopPropagation(); onActionClick(); };
    updateActionButton();
  }
  newBtn.onclick = () => {
    resetForNewSession();
  ...
  if (modeBtn) modeBtn.onclick = (e) => { e.stopPropagation(); openModePopover(); };
  if (modelBtn) modelBtn.onclick = (e) => { e.stopPropagation(); openModelPopover(); };
  if (uploadBtn) uploadBtn.onclick = (e) => { e.stopPropagation(); vscode.postMessage({ type: "pickFile" }); };
  ...
  const verBoot = $("welcome-version");
  if (verBoot) {
    verBoot.classList.add("loading-dots");
    verBoot.textContent = "Starting";
  }
  state.startingPhase = true;
