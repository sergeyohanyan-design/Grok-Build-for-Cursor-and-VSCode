...
describe("unified action button (mic / send)", () => {
  it("shows mic when empty and send when the composer has text", () => {
    const { doc, window } = bootWebview();
    const btn = $(doc, "action-btn");
    const input = $(doc, "input") as HTMLTextAreaElement;

    expect(btn.classList.contains("mic-mode")).toBe(true);
    expect(btn.classList.contains("send-mode")).toBe(false);

    input.value = "hello";
    input.dispatchEvent(new (window as any).Event("input", { bubbles: true }));

    expect(btn.classList.contains("send-mode")).toBe(true);
    expect(btn.classList.contains("mic-mode")).toBe(false);
  });

  it("clicking send mode posts a message instead of starting voice", () => {
    ...
    expect(posted.some((p) => p.type === "voiceStart")).toBe(false);
  });
});

describe("voice input action button", () => {
  it("starts idle showing the mic icon", () => {
    const { doc } = bootWebview();
    const mic = $(doc, "action-btn");
...
describe("voice input: live streaming transcription", () => {
  ...
  it("with text in the composer, the action button sends instead of starting voice", () => {
    const { window, posted, doc } = bootWebview();
    const btn = $(doc, "action-btn");
    const input = $(doc, "input") as HTMLTextAreaElement;
    input.value = "Note:";
...
describe("voice input: continuous listening + queue (hands-free)", () => {
  it("voiceSubmit sends immediately when idle, clears composer, and KEEPS listening", () => {
    ...
    expect(mic.classList.contains("stop")).toBe(true);         // sending — mic stream still live on host
...
describe("voice input: API-key setup hint", () => {
  it("shows a 'needs setup' hint when the host reports no key", () => {
    ...
    expect(mic.title.toLowerCase()).toContain("needs setup");
  });
